...
export default function WalletModal({ isOpen, onClose, onConnect }: WalletModalProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      StellarWalletsKit.init({ modules: MODULES, network: Networks.TESTNET });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4 sm:items-center">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <p className="mb-4 text-sm text-gray-500">Select a Stellar wallet to connect.</p>
            ...
            return (
              <li key={wallet.id}>
                <button
                  ...
                  disabled={isLoading}
                  className={`flex w-full min-h-[48px] items-center gap-3 rounded-lg border px-4 py-3 text-left transition-colors ${
                    !wallet.isAvailable
