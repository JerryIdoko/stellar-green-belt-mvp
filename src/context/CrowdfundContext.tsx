...
export function CrowdfundProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    trackEvent("Wallet Connected", { address: address.slice(0, 8) });
    refreshCampaign();
  }, [address]);
  ...
  const contribute = useCallback(
    async (amount: number) => {
      ...
      trackEvent("Transaction Initialized", { amount, address: address.slice(0, 8) });
      ...
      try {
        ...
        if (err instanceof Error) {
          ...
          console.error("Contribute Error:", {
            method: "fund",
            contractId: CONTRACT_ID,
            params: { donor: address.slice(0, 8), amount },
